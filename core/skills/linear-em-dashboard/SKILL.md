...
description: >
  ...
  velocity, ticket lifespan, carry-over rate, scope growth, and per-assignee throughput for
...
| Ticket lifespan | Median days from `startedAt → completedAt` for completed issues |
...
- `description`: "Live Engineering Manager dashboard — team-switchable. Fetches velocity,
  ticket lifespan, carry-over rate, scope growth, and assignee throughput from Linear each
  time it is opened."
...
- Ticket lifespan only counts issues that have a `startedAt` timestamp (i.e. were actively
...
- **Lifespan**: uses `startedAt → completedAt`; issues without `startedAt` excluded from median
