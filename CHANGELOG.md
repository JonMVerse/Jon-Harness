...
- **R2 — confidence gate removed from `code-reviewer`.** The `>80%` reporting gate (present in four places) was obeyed literally on Opus 5 and dropped real bugs while `/review` asked for ALL issues. `code-reviewer` and `security-reviewer` now report every finding with a 0–100 confidence score (`tech-debt-reviewer` stays severity-only and writes its own `.debt` report); the `/review` consolidation is the single triage pass (ordered by severity, then confidence where provided). The merge verdict rests on confident findings (≥75) — a low-confidence CRITICAL is flagged for a human look rather than blocking on its own. Removes the double-filter (find-time + synthesis-time).
...
- **R7 — explicit `effort` on all agents.** The six agents that inherited session effort now declare it (11/11 coverage): `code-reviewer`/`security-reviewer`/`code-explorer`/`tech-debt-reviewer`/`test-generator` `medium`, `documentation-generator` `low`. Both reviewers were **eval-swept** with 18-run planted-defect sweeps (`plans/2026-07-31-code-reviewer-effort-sweep/`, `plans/2026-07-31-security-reviewer-effort-sweep/`): recall/precision saturated at `low` at every effort, `high` bought nothing (only mild severity inflation on the code-reviewer), so both settled at `medium` — a margin above the corpus's clean signal for a review gate. `code-explorer`/`tech-debt-reviewer`/`test-generator`/`documentation-generator` remain reasoned defaults pending their own corpora.
...
- **`/linear-build`**: new command. Orchestrates delivery from an agent-ready board with a human in the loop at both ends — walks the dependency graph for unblocked tickets, ensures the routing labels exist on the team and are applied, lints each ticket against the readiness standard, widens the lens beyond the ticket (file/contract overlap, product-vs-tech implications surfaced to the human, never resolved silently), human-gates auth/personal-data/spend/migration work and anything under an open privacy action, then **proposes an implementation outline (approach, adversarially-argued alternatives, wider impact, verification) for explicit human green light before any code is written**. Approved plans compile into one-shot briefs routed to `mech-executor`/`executor` (scout pass when grounding is thin); output must survive an adversarial `verifier` pass **and** a `/review`-style fan-out (code/security/tech-debt reviewers, fix loop capped at two rounds) before a PR is pushed; the PR carries the plan + evidence and requests human review — the agent never merges. Adversarial stance applies at every stage: each decision names its strongest alternative and why it loses, and no green light is requested bare — every gate states the risk of each choice, the cost of not proceeding, the blast radius, and the rollback path. **Hard-stop at review boundaries is the default**: a ticket blocked by another that is only in review (PR pushed, not yet merged) is not eligible; the human merge re-arms the chain, so every PR is cut against `main` and stacked-PR rebase cascades are avoided. The loop fills the wait with independent tickets and stops when everything eligible is awaiting a merge, on consecutive blocks/refutations, or at a passed milestone date — then recommends re-planning. `--allow-soft-deps` opts into stacked branches (softness named in plan + PR; upstream rebase re-verifies downstream) for solo maintainers who review quickly; `--dry-run` reports the plan with no Linear writes, dispatch, or PRs.
