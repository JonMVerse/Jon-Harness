...
- **R7 — explicit `effort` on all agents.** The six agents that inherited session effort now declare it (11/11 coverage): `security-reviewer` `high`, `code-reviewer`/`code-explorer`/`tech-debt-reviewer`/`test-generator` `medium`, `documentation-generator` `low`. `code-reviewer` was **eval-swept** (18-run planted-bug sweep, `plans/2026-07-31-code-reviewer-effort-sweep/`): recall/precision saturated at `low`, `high` only inflated severities, so it settled at `medium` (a margin above the corpus's clean signal for the merge gate). The rest remain reasoned defaults pending their own corpora.
